...
export function renderPlayer(i) {
    // Eliminated FFA fighters are still simulated but not drawn.
    if (player[i].dead) {
        return;
    }
    // Source the render transform from the dynamic 100-man camera when active,
    // otherwise fall back to the static stage transform (normal play).
    var cam = (typeof window !== "undefined") ? window.__cam : null;
    var scale = cam ? cam.scale : activeStage.scale;
    var offX = cam ? cam.offsetX : activeStage.offset[0];
    var offY = cam ? cam.offsetY : activeStage.offset[1];
    var temX = (player[i].phys.pos.x * scale) + offX;
    var temY = (player[i].phys.pos.y * -scale) + offY;
    // Cull fighters whose screen position is off-canvas (1200x750 design space).
    // Only when the dynamic camera is active (100-man), to preserve the normal
    // game's off-screen mini-view arrow indicators.
    if (cam) {
        var canvasW = 1200, canvasH = 750;
        if (temX < -100 || temX > canvasW + 100 || temY < -100 || temY > canvasH + 100) {
            return;
        }
    }
    ...
    if (!actionStates[characterSelections[i]][player[i].actionState].dead) {
        ...
        if (temX > 1220 || temX < -20 || temY > 880 || temY < -30) {
            ...
            if (-375 <= s * 600 && s * 600 <= 375) {
                ...
                if (pA.y > pB.y) {
                    ...
                    player[i].miniViewPoint = new Vec2D(-375 / s + offX, 50);
                    player[i].miniViewSide = 2;
                }
                player[i].miniView = true;
                player[i].phys.outOfCameraTimer++;
        ...
        if (player[i].miniView && player[i].actionState != "SLEEP") {
            ...
            if (player[i].actionState == "ENTRANCE") {
                drawArrayPathCompress(fg2, col, face, temX, temY, model, player[i].charAttributes.charScale * (scale /
                    4.5), Math.min(player[i].charAttributes.charScale, player[i].charAttributes.charScale * (1.5 -
                        startTimer)) * (scale / 4.5), player[i].rotation, player[i].rotationPoint.x, player[i].rotationPoint
                    .y);
            } else {
                drawArrayPathCompress(fg2, col, face, temX, temY, model, player[i].charAttributes.charScale * (scale /
                    4.5), player[i].charAttributes.charScale * (scale / 4.5), player[i].rotation, player[i].rotationPoint
                    .x, player[i].rotationPoint.y);
            }
        }
    }
    if (player[i].phys.shielding) {
        if (!(player[i].phys.powerShielded && player[i].hit.hitlag > 0)) {
            var sX = ((player[i].phys.shieldPositionReal.x) * scale) + offX;
            var sY = ((player[i].phys.shieldPositionReal.y) * -scale) + offY;
            ...
            fg2.beginPath();
            fg2.arc(sX, sY, player[i].phys.shieldSize * scale, twoPi, 0);
            fg2.fill();
        }
    }
    if (hasTag[i]) {
        ...
        fg2.fillRect(temX - size / 2, temY - 130 * (scale / 4.5), size, 20);
        fg2.strokeRect(temX - size / 2, temY - 130 * (scale / 4.5), size, 20);
        fg2.font = "13px Lucida Console, monaco, monospace";
        fg2.textAlign = "center";
        fg2.fillStyle = "white";
        fg2.fillText(tagText[i], temX, temY + 15 - 130 * (scale / 4.5));
        fg2.fillStyle = palettes[pPal[i]][0];
        fg2.beginPath();
        fg2.moveTo(temX - 8, temY + 20 - 130 * (scale / 4.5));
        fg2.lineTo(temX + 8, temY + 20 - 130 * (scale / 4.5));
        fg2.lineTo(temX, temY + 28 - 130 * (scale / 4.5));
        fg2.closePath();
        fg2.fill();
        fg2.textAlign = "start";
    }
    if (player[i].actionState == "REBIRTH" || player[i].actionState == "REBIRTHWAIT") {
        ...
        fg2.strokeStyle = palettes[pPal[i]][0];
        fg2.beginPath();
        fg2.moveTo(temX + 18 * (scale / 4.5), temY + 13.5 * (scale / 4.5));
        fg2.lineTo(temX + 31.5 * (scale / 4.5), temY);
        fg2.lineTo(temX - 31.5 * (scale / 4.5), temY);
        fg2.lineTo(temX - 18 * (scale / 4.5), temY + 13.5 * (scale / 4.5));
        fg2.closePath();
        fg2.fill();
        fg2.stroke();
    }
    if (player[i].showLedgeGrabBox) {
        ...
        fg2.strokeRect(player[i].phys.ledgeSnapBoxF.min.x * scale + offX, player[i].phys.ledgeSnapBoxF.min
                .y * -scale + offY, 14 * scale, 10 * scale);
        fg2.strokeStyle = "#ff4444";
        fg2.strokeRect(player[i].phys.ledgeSnapBoxB.min.x * scale + offX, player[i].phys.ledgeSnapBoxB.min
                .y * -scale + offY, 14 * scale, 10 * scale);
    }
    if (player[i].showECB) {
        fg2.fillStyle = "#ff8d2f";
        fg2.beginPath();
        fg2.moveTo((player[i].phys.ECB1[0].x * scale) + offX, (player[i].phys.ECB1[0].y * -scale) +
            offY);
        fg2.lineTo((player[i].phys.ECB1[1].x * scale) + offX, (player[i].phys.ECB1[1].y * -scale) +
            offY);
        fg2.lineTo((player[i].phys.ECB1[2].x * scale) + offX, (player[i].phys.ECB1[2].y * -scale) +
            offY);
        fg2.lineTo((player[i].phys.ECB1[3].x * scale) + offX, (player[i].phys.ECB1[3].y * -scale) +
            offY);
        fg2.closePath();
        fg2.fill();
        fg2.strokeStyle = "white";
        fg2.beginPath();
        fg2.moveTo((player[i].phys.ECBp[0].x * scale) + offX, (player[i].phys.ECBp[0].y * -scale) +
            offY);
        fg2.lineTo((player[i].phys.ECBp[1].x * scale) + offX, (player[i].phys.ECBp[1].y * -scale) +
            offY);
        fg2.lineTo((player[i].phys.ECBp[2].x * scale) + offX, (player[i].phys.ECBp[2].y * -scale) +
            offY);
        fg2.lineTo((player[i].phys.ECBp[3].x * scale) + offX, (player[i].phys.ECBp[3].y * -scale) +
            offY);
        fg2.closePath();
        fg2.stroke();
        fg2.beginPath();
    ...
    if (player[i].showHitbox) {
        ...
        fg2.fillRect(player[i].phys.hurtbox.min.x * scale + offX, player[i].phys.hurtbox.min.y * -scale +
            offY, player[i].charAttributes.hurtboxOffset[0] * 2 * scale, player[i].charAttributes.hurtboxOffset[
                1] * scale);
        ...
        for (var j = 0; j < 4; j++) {
            ...
            if (player[i].hitboxes.active[j]) {
                ...
                fg2.beginPath();
                fg2.arc(((offset.x * player[i].phys.face + player[i].phys.pos.x) * scale) + offX, ((offset.y +
                    player[i].phys.pos.y) * -scale) + offY, player[i].hitboxes.id[j].size * scale,
                    Math.PI * 2, 0);
                fg2.fill();
                if (player[i].phys.prevFrameHitboxes.active[j]) {
                    ...
                    fg2.beginPath();
                    fg2.arc(((offset.x * player[i].phys.facePrev + player[i].phys.posPrev.x) * scale) + offX,
                        ((offset.y + player[i].phys.posPrev.y) * -scale) + offY, player[i].phys.prevFrameHitboxes
                            .id[j].size * scale, Math.PI * 2, 0);
                    fg2.fill();

                    //console.log(player[i].phys.interPolatedHitbox[j]);
                    fg2.beginPath();
                    fg2.moveTo((player[i].phys.interPolatedHitbox[j][0].x * scale) + offX, (player[i].phys.interPolatedHitbox[
                            j][0].y * -scale) + offY);
                    fg2.lineTo((player[i].phys.interPolatedHitbox[j][1].x * scale) + offX, (player[i].phys.interPolatedHitbox[
                            j][1].y * -scale) + offY);
                    fg2.lineTo((player[i].phys.interPolatedHitbox[j][2].x * scale) + offX, (player[i].phys.interPolatedHitbox[
                            j][2].y * -scale) + offY);
                    fg2.lineTo((player[i].phys.interPolatedHitbox[j][3].x * scale) + offX, (player[i].phys.interPolatedHitbox[
                            j][3].y * -scale) + offY);
                    fg2.closePath();
                    fg2.fill();
                    fg2.stroke();
